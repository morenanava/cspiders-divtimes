...
QC_conv <- function( num_dirs, delcol, path, num_divt, node_calib, dataset, perc, 
  ...
  rownames( ESS_results_table ) <- c( "median", "min", "max", "num_samples_for_ESS",
                                      "tail-ESS_med", "tail-ESS_min", "tail-ESS_max",
                                      "bulk-ESS_med", "bulk-ESS_min", "bulk-ESS_max",
  ...
  cat( "\n    * * *    \n\n")
  # Output table
  if( dir.exists( paste( outchecks_dir, "../out_RData/", sep = "" ) ) == FALSE ){
